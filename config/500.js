...
module.exports[500] = function serverErrorOccurred(errors, req, res,
	...
	var displayedErrors = (typeof errors !== 'object' || !errors.length) ? [ errors ]
			: errors;
	...
	for ( var i in displayedErrors) {
		...
		loggers.newrelic.error(req.url, res.statusCode, displayedErrors[i].message, displayedErrors[i].stack);
		loggers.udp.error(displayedErrors[i].message + " | STACK: "	+ displayedErrors[i].stack);
	}
	
	// In production, don't display any identifying information about the
	// error(s)
